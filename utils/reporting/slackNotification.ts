...
export class SlackNotification implements INotificationReporter {
  ...
  async prepareNotificationJson(results: any): Promise<void> {
    ...
    try {
      ...
      const jenkinsJobLink = await fs.readFileSync(this.allureReportLink, 'utf-8');
      // const whoTriggeredJob = await fs.readFileSync(whoTriggeredJobFile, 'utf-8');
      const branchName = await fs.readFileSync(this.branchNameFile, 'utf-8');
      ...
      const dataFileContent = await fs.readFileSync(this.notificationJsonFile);
      ...
      await fs.writeFileSync(this.notificationJsonFile, JSON.stringify(parseJson));
    } catch (error) {
      throw new Error(error);
    }
  }
