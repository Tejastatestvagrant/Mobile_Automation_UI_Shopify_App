...
then
    ...
    echo "executing test-ui from package.json"
    Group="${Group}" Env="${Env}" Platform="${Platform}" AppUrl="${AppUrl}" Headless="${Headless}" SendNotification="${SendNotification}" npm run test-ui
fi
