...
class SwipeUtil extends baseScreen_1.BaseScreen {
     ...
     * @param {number} amount
     */
    async checkIfVisibleWithScrollDown(element, maxScrolls, amount = 0) {
        const webElement = await this.getElement(element);
        if ((!(await webElement.isExisting()) || !(await webElement.isDisplayed()))
            && amount <= maxScrolls) {
    ...
    async swipeDown(percentage = 1) {
        await this.swipeOnPercentage(await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.down.start, percentage), await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.down.end, percentage));
    }
    /**
    ...
    async swipeUp(percentage = 1) {
        await this.swipeOnPercentage(await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.up.start, percentage), await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.up.end, percentage));
    }
    /**
    ...
    async swipeLeft(percentage = 1) {
        await this.swipeOnPercentage(await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.left.start, percentage), await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.left.end, percentage));
    }
    /**
    ...
    async swipeRight(percentage = 1) {
        await this.swipeOnPercentage(await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.right.start, percentage), await SwipeUtil.calculateXY(this.SWIPE_DIRECTION.right.end, percentage));
    }
    /**
    ...
    async swipeOnPercentage(from, to) {
        ...
        webservicesExport_1.LOGGER.info(`SCREEN_SIZE {width:${this.SCREEN_SIZE.width}, height:${this.SCREEN_SIZE.height}}`);
     ...
     * @return {{x: number, y: number}}
     */
    static async calculateXY({ x, y }, percentage) {
        return {
            x: x * percentage,
            y: y * percentage,
        };
